...
jest.mock('bcryptjs', () => ({
    hash: jest.fn().mockImplementation((password: string, _saltRounds: number) => Promise.resolve(`hashed_${password}`))
}));
...
describe('User Class', () => {
    ...
    it('should throw error when hashPassword method fails', async () => {
        (bcrypt.hash as jest.Mock).mockRejectedValue(new Error('Mocked hash error'));
